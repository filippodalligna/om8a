...
def register():
    ...
    if not data or not data.get('username') or not data.get('email') or not data.get('password'):
        return jsonify({'message': _('Missing username, email, or password')}), 400
    ...
    if User.query.filter_by(username=username).first():
        return jsonify({'message': _('Username already exists')}), 409
    if User.query.filter_by(email=email).first():
        return jsonify({'message': _('Email already exists')}), 409
...
def login():
    ...
    if not data or (not data.get('email') and not data.get('username')) or not data.get('password'):
        return jsonify({'message': _('Missing email/username or password')}), 400
